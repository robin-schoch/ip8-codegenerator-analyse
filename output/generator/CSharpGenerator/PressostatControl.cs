...
namespace Swissframe.Util.StateEventEditor.Common.Test
{
    ...
    public class PressostatControl
    {
        ...
        private bool Eventf25ca455()
        {
            var result = (_registry.Variable_PressostatControl_PauseCount  > _registry.Variable_PressostatControl_PauseCount);
            _logger.LogDebug("Event: Eventf25ca455() evaluates to {result}", result);
            return result;
        } 
        private bool Event8a44a3ac()
        {
            ...
            _logger.LogDebug("Event: Event8a44a3ac() evaluates to {result}", result);
            return result;
        } 
        private bool Eventdcf263c2()
        {
            var result = (_registry.Timer_PressostatControl_Pause  >= _registry.Timer_PressostatControl_Pause);
            _logger.LogDebug("Event: Eventdcf263c2() evaluates to {result}", result);
            return result;
        } 
    }
}