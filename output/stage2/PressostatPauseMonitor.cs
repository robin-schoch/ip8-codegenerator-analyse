...
namespace Swissframe.Util.StateEventEditor.Common.Test
{
    ...
    public class PressostatPauseMonitor
    {
        ...
        private bool Event6()
        {
            var result = (_registry.Variable_PressostatMonitor_PauseCount  > _registry.Constant_Zero);
            _logger.LogDebug("Event: Event6() (f38d9bb9-5a8f-4170-9bb5-3bc81c0ffcb8) evaluates to {result}", result);
            return result;
        } 

        private bool Event8()
        {
            var result = (_registry.Variable_PressostatMonitor_PauseCount  >= _registry.Config_PressostatPauseMonitorConfiguration::MaximumPauseCount);
            _logger.LogDebug("Event: Event8() (6ea8d615-2496-43ef-88da-cc68e015221b) evaluates to {result}", result);
            return result;
        } 

        private bool Event7()
        {
            var result = (_registry.Timer_PressostatPauseMonitor_Active  >= _registry.Config_PressostatPauseMonitorConfiguration::MaximumActiveDurationHours);
            _logger.LogDebug("Event: Event7() (8f099316-96dc-4d46-9702-0814ac17a61e) evaluates to {result}", result);
            return result;
        } 
